...
"""
        ...
        # 写入ass
        output_path = os.path.splitext(xml_name)[0] + '.ass'
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        for i, file_path in enumerate(sys.argv[1:], 1):
            print(f"\n处理文件 [{i}/{len(sys.argv)-1}]: {file_path}")
            ...
            if not (file_path.lower().endswith('.xml') or file_path.lower().endswith('.json')):
                print(f"错误: 仅支持XML或JSON文件 - {file_path}")
                continue
                
            success, result = xml2ass(file_path)
            if success:
                print(f"✓ 转换成功: {result}")
            else:
                print(f"✗ {result}")
    else:
        print("\n请将XML文件拖放到此程序图标上而不是打开该程序")
